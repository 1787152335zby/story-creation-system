              ...
              <div key={cfg.id} className={`premium-subpanel overflow-hidden transition-all duration-300 ${cfg.active ? 'ring-2 ring-primary/40' : ''}`}>
                <div className="p-4">
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {cfg.active && <span className="text-[10px] text-primary font-semibold px-1.5 py-0.5 rounded-full bg-primary/10">当前</span>}
                        </div>
                      </div>
                    )}
                    {!isEditing && (
                  ...
                  {isEditing ? (
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[10px] text-muted-foreground font-mono truncate flex-1">{cfg.base_url}</span>
                      <button onClick={() => setVisibleAggKey(visibleAggKey === cfg.id ? null : cfg.id)}
                        className="text-[10px] text-muted-foreground font-mono bg-muted px-1.5 py-0.5 rounded flex items-center gap-1 hover:text-foreground transition-colors flex-shrink-0">
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-1.5 pt-1 border-t border-border/40">
                      <button onClick={async () => {
                        ...
                        className="btn-gradient flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-medium">
                        <Save className="w-2.5 h-2.5" /> 保存
                      </button>
                    ...
                    <div className="flex items-center gap-1 pt-1 border-t border-border/40">
                      ...
                      {cfg.active ? (
                        <span className="flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] font-medium text-primary border border-primary/30 bg-primary/10">
                          <Check className="w-2.5 h-2.5" /> 使用中
                        </span>
                      ) : (
                        <button onClick={() => { activateAggConfig(cfg.id).then(() => fetchAggConfigsAndModels()) }}
                          className="flex items-center gap-0.5 px-2 py-1 rounded-lg border border-accent/30 text-[10px] text-accent hover:bg-accent/10 transition-colors">
                          <Check className="w-2.5 h-2.5" /> 使用
                        </button>
                      )}
                      <button onClick={() => { if (!confirm('确定删除？')) return; deleteAggConfig(cfg.id).then(() => fetchAggConfigsAndModels()) }}
                        className="flex items-center gap-0.5 px-2 py-1 rounded-lg border border-red-400/30 text-[10px] text-red-400 hover:bg-red-400/10 transition-colors ml-auto">
                  ...
                  {aggTestResultSet[cfg.id] && (
                    <div className={`flex items-center gap-1 p-1.5 mt-1.5 rounded-lg text-[10px] ${aggTestResultSet[cfg.id].success ? 'bg-green-400/5 border border-green-400/20' : 'bg-red-400/5 border border-red-400/20'}`}>
